...
reg1 <- lm(Produkce ~ Investice, data=Firmy)
summary(reg1)   # vystup
...
hist(reg1$residuals)  # histogram rezidui
shapiro.test(reg1$residuals)  # test normality rezidui
...
mahal6 <- mahalanobis(Cars_data2, xbarC, cov_xC, inverted = FALSE)
plot(density(mahal6, bw = 0.3), main="Squared Mahalanobis distancec"); rug(mahal6)
which(mahal6>12) 
write.table(mahal6, "c:/R/Data/mahal6.txt", sep="\t",dec = ",")

 # 2. Jacknife rezidua
jack6 <-studres(reg6)
ggplot(reg6,aes(x=.fitted,y=jack6)) + geom_point() +
...
reg6_2 <- lm(Cena_prodej ~ Motor + Valce + Horsepower + Mesto_MPG + Dalnice_MPG + Vaha + Delka, data=Cars_data)
summary(reg6_2)
reg6_3 <- lm(Cena_prodej ~ Motor + Valce + Horsepower + Dalnice_MPG + Vaha + Delka, data=Cars_data)
summary(reg6_3)
